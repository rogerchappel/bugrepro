...
export function validateManifest(value: unknown): asserts value is ReproManifest {
  ...
  if (!manifest.command || typeof manifest.command !== 'object') throw new Error('Manifest command is missing');
  if (!Array.isArray(manifest.command.command) || manifest.command.command.length === 0) {
    throw new Error('Manifest command.command must be a non-empty array of strings');
  }
  if (!manifest.command.command.every((part) => typeof part === 'string')) {
    throw new Error('Manifest command.command must be a non-empty array of strings');
  }

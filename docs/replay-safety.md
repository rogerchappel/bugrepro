...
`bugrepro replay` runs the captured command from the bundle's `fixtures/` directory when fixtures were captured. For a bundle with no fixtures, it runs from the bundle directory itself so replay always uses a defined, existing working directory. It skips the confirmation prompt only for `cat` and `grep` commands that do not contain shell-control characters or known dangerous command names.
